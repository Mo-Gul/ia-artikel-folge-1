...
Dokumentation liegt in Office-Formaten, etwa docsx, pptx oder xlsx vor.
...
Für einfache UML-Diagramme schlagen wir Ihnen eine elegante Lösung namens PlantUML vor:
...
und somit die Callouts beim Kopieren des Textes in eigenen Code manuell entfehrnt werden müssen.
Sie sollten also bei Beispielen eher sparsam eingesetzt werden.

=== Textkästen (Sidebars)

Manchmal möchten Sie Inhalte besonders hervorheben.
Gerade bei technische Dokumentation bietet es sich an,
den einen oder anderen Sachverhalt ausserhalb des Fliesstextes zu erklären.
In einem solchen Fall sollten Sie die Syntax für eine Sidebar verwenden.
...
Beachten Sie, dass durch die Definition der Anzahl der Spalten in den Optionen,
die Zellen einfach untereinander definiert werden können.
...
Auf dieser Website können sie direkt AsciiDoc in einem Editor (links) eingeben
und sehen das erzeugte HTML auf der rechten Seite.
In diesem Fall kommt die JavaScript-Version von Asciidoctor zum Einsatz.

Wenn Sie über die Kommandozeile die Konvertierung vornehmen wollen, dann gibt es z.B. `asciidoctorj`, welches Sie am einfachsten über https://sdkman.io/[sdkman.io] installieren.

Zur Umsetzung des Docs-as-Code Ansatzes bauen Sie die Konvertierung direkt in Ihren Software-Build ein und bauen Ihre Dokumentation von nun an genauso wie Ihre Software.
Dazu exitieren Plugins für z.B. Gradle und Maven.
...
In unserem https://github.com/doctoolchain/ia-artikel[Github-Repository] finden Sie ein etwas erweitertes Beispiel, welches schon plantUML unterstützt.

Übrigens - wir verwenden sehr gerne Gradle für den Docs-as-Code Ansatz.
Sollte Ihr Projekt ein anderes Build-Tool verwenden, so lassen sich die Tools auch gut mischen.
Einem Bau der Software mit Maven und dem Bau der Dokumentation mit Gradle spricht nicht viel entgegen.

Sollte Ihnen HTML als Ausgabeformat nicht reichen, so untestüzt Asciidoctor direkt noch docBook
und über ein Plugin PDF.
Wie sie weitere Ausgabeformate erzeugen können, zeigen wir Ihnen dann im zweiten Teil.

IMPORTANT: fehlt noch

// wir sollten noch ein paar (kurze) Worte zum erzeugen von Ausgabeformaten
// schreiben - wie das zB mit gradle oder per Kommandozeile geht,
// und dass man diesen "Doku-Build" daher auch in den "Source-Build"
// integrieren kann. Über Testen von Doku können wir ja in Folge 2 schreiben,
// oder das auf das Booklet und die Website verschieben.
...
// @RDM -> einen Verweis auf Dein EAP-Extraktions-Skript??
// @GS -> zweiter Teil
****
...
Mit dem in diesem Artiekl beschriebenen Ansatz sind Sie schon voll gerüstet für Docs-as-Code.
In der zweiten Folge dieses Artikels zeigen wir Ihnen, wie noch einen Schritt weiter gehen
und mit Hilfe der frei verfügbaren Werkzeugkette docToolchain,
Office-Dokumente integrieren, Word(TM) und Confluence(TM) Formate generieren und noch
andere spannende Dinge erledigen können. Eben das, was Sie auch mit Code machen würden: extrahieren, generieren, transformieren, testen etc.
...
NOTE: @GS brauchen wir die? Wir sind online, die meisten Quellen sind verlinkt, oder?
