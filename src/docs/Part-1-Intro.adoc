...
und Quellcode.
2. In der zweiten Folge lernen Sie XXX[docToolchain] kennen, die OpenSource Lösung zur Unterstützung des Docs-As-Code Ansatzes.
...
Site wird nach dem Motto EYOD (_eat your own dogfood_) generiert,
sämtliche Quellen liegen unter Versionsverwaltung und
...
XXX-Referenz-JohnGruber[Markdown] handelt sich bei AsciiDoc um einen wirklichen Standard,
...
, und erfüllt
damit sämtliche oben genannten  Anforderungen an Dokumentation. Bei Bedarf
  können Sie sogar Bücher damit schreiben.).
...
image::docs-as-code-uebersiht.png[]
